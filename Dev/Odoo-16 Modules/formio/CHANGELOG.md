...
Implement the reset of installed formio.js versions (download and reinstall of formio.js assets).\
This also improves the actual dowload and reinstall by keeping the formio.version but replacing the formio.js assets.\
Also, the default CSS assets (e.g. bootstrap CSS) are no longer removed, so manual addition/repair no longer necessary.
